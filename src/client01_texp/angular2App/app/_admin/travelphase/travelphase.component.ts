...
import { TravelphaseService } from './travelphase.service';
...
@Component({
    templateUrl: 'travelphase.component.html'
    , providers: [TravelphaseService
        , DataListModule
        , DataTableModule
        , ContextMenuModule
        , DialogModule
...
export class TravelphaseComponent implements OnInit {
    ...
    constructor(private _Service: TravelphaseService) { }
    ...
    onSubmit() {
        if (this.selected.travelphaseId != undefined) { //update
    ...
    delete() {
        this._Service
            ...
            .then(() => {
                this.list = this.list.filter(h => h.travelphaseId !== this.selected.travelphaseId);
                this.selected = {};
            });
    }
}
